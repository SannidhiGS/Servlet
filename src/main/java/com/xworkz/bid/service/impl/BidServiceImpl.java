...
//        }
    ...
    @Override
    public String placeBid(String playerName, String teamEmail, double bidAmount) throws SQLException, ClassNotFoundException {
        bidDAO.saveBid(playerName, teamEmail, bidAmount);
        int count = bidDAO.getBidCountForPlayer(playerName);

        if (count >= 3) {
            BidDTO highest = bidDAO.getHighestBid(playerName);
            bidDAO.allocatePlayer(playerName, highest.getTeamEmail());
            return "Player " + playerName + " allocated to team " + highest.getTeamEmail() + " for amount " + highest.getBidAmount();
        } else {
            return "Bid placed successfully for " + playerName + ". Total bids: " + count;
        }
    }
